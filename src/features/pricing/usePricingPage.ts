      ...
      for (const r of validRows) {
        if (r.variantId) {
          const { error: vError } = await supabase
            .from("variants")
            .update({
              // Mantenemos `price` por compatibilidad y guardamos los 3 niveles
              price:         Number(r.finalPrice),
              price_menudeo: Number(r.suggestedPrices?.menudeo ?? r.finalPrice),
              price_medio:   Number(r.suggestedPrices?.medio   ?? r.finalPrice),
              price_mayoreo: Number(r.suggestedPrices?.mayoreo ?? r.finalPrice),
            })
            .eq("id", r.variantId);
