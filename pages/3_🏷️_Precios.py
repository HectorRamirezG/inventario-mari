...
tab_calc, tab_config, tab_hist = st.tabs(["🧮 Calculadora", "⚙️ Configuración", "📜 Historial"])
...
# ════════════════════════════════════
with tab_config:
    section("Márgenes y umbrales", icon="⚙️")
    with st.form("cfg-form"):
        ...
        if st.form_submit_button("💾 Guardar configuración", type="primary", use_container_width=True):
            if u_med >= u_may:
                ...
                st.warning("⚠️ Recomendado: margen menudeo > medio > mayoreo. Lo guardo igual.")
